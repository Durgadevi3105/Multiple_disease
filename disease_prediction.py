...
if nav == "Parkinson's Disease":
    ...
    try:
        parkinson_model = pickle.load(open(r'XGBparkinsons.pkl', 'rb'))
    ...
    # Prepare input features as a 2D array for prediction
    input_features = np.array([[Age, Blood_Pressure, Specific_Gravity, Albumin, Sugar,
                            1 if Red_Blood_Cells == "abnormal" else 0,
                            1 if Pus_Cell == "abnormal" else 0,
                            ...
                            1 if Anemia == "yes" else 0]]).astype(float)
